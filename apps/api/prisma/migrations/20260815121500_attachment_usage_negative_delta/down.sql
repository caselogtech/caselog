...
DECLARE
    ...
    size_delta BIGINT := 0;
BEGIN
    IF TG_OP = 'UPDATE' AND (NEW."organization_id" <> OLD."organization_id" OR NEW."checksum_sha256" <> OLD."checksum_sha256") THEN
    ...
    END IF;
    IF reference_delta = 0 THEN IF TG_OP = 'DELETE' THEN RETURN OLD; END IF; RETURN NEW; END IF;
    ...
    WHERE blob."organization_id" = target_organization_id AND blob."checksum_sha256" = target_checksum
    RETURNING blob."active_reference_count", CASE WHEN blob."storage_status" <> 'missing'
...
BEGIN
    ...
    IF (CASE WHEN TG_OP = 'DELETE' THEN OLD."active_reference_count" ELSE NEW."active_reference_count" END) > 0 THEN
      IF TG_OP <> 'INSERT' AND OLD."storage_status" <> 'missing' THEN previous_size := COALESCE(OLD."storage_observed_size_bytes", OLD."size_bytes"); END IF;
      IF TG_OP <> 'DELETE' AND NEW."storage_status" <> 'missing' THEN current_size := COALESCE(NEW."storage_observed_size_bytes", NEW."size_bytes"); END IF;
...
DROP FUNCTION "caselog"."apply_attachment_storage_usage_delta"(UUID, BIGINT);
