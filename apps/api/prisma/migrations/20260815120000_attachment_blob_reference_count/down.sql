...
BEGIN
    ...
    SELECT CASE WHEN blob."storage_status" <> 'missing'
    ...
    IF TG_OP = 'INSERT' AND NEW."deleted_at" IS NULL AND NOT EXISTS (
      SELECT 1 FROM "attachments" WHERE "organization_id" = target_organization_id
        AND "checksum_sha256" = target_checksum AND "deleted_at" IS NULL AND "id" <> NEW."id"
    ) THEN size_delta := physical_size;
    ELSIF TG_OP = 'DELETE' AND OLD."deleted_at" IS NULL AND NOT EXISTS (
      SELECT 1 FROM "attachments" WHERE "organization_id" = target_organization_id
        AND "checksum_sha256" = target_checksum AND "deleted_at" IS NULL
    ) THEN size_delta := -physical_size;
...
BEGIN
    ...
    IF EXISTS (
      SELECT 1 FROM "attachments" WHERE "organization_id" = target_organization_id
        AND "checksum_sha256" = CASE WHEN TG_OP = 'DELETE' THEN OLD."checksum_sha256" ELSE NEW."checksum_sha256" END
...
ALTER TABLE "attachment_blobs"
    DROP CONSTRAINT "chk_attachment_blobs_active_reference_count",
    DROP COLUMN "active_reference_count";
