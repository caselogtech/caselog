...
BEGIN
    ...
    IF TG_OP <> 'INSERT' AND OLD.deleted_at IS NULL AND OLD.storage_status = 'healthy' THEN previous_size := OLD.size_bytes; END IF;
    IF TG_OP <> 'DELETE' AND NEW.deleted_at IS NULL AND NEW.storage_status = 'healthy' THEN current_size := NEW.size_bytes; END IF;
...
ALTER TABLE "attachments"
    DROP CONSTRAINT IF EXISTS "chk_attachments_storage_observed_size_bytes",
    DROP COLUMN IF EXISTS "storage_observed_size_bytes";
